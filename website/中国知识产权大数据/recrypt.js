...
function encrypt(passwd) {
    var result = encryptedString(key, encodeURIComponent(passwd), RSAAPP.NoPadding, RSAAPP.RawEncoding);
    encrypt_base64 = Buffer.from(result,'binary').toString('base64');
    return encrypt_base64;
}


bodyRSA();
console.log(encrypt('dsadad'));

// btoa和atob是window对象的两个函数，其中btoa是bianry to AscII，用于将binary的数据用ASCII码表示，即Base64的编码过程，而atob则是ASCII to binary，用于将ASCII码解析成binary数据，即Base64解码过程
// binary是js的另一个子集，它类似与ASCII字符集，但是字符的码点(charCode)不再限制到127，它包含了255以内的字符。binary string设计的目的不是用于代表字符，而是代表二进制数据。由binary string代表
// 的二进制数据大小是原始数据的两倍，然而这对于最终用户是不可见的，因为JavaScript strings的长度是2字节为单位进行计算的比如， "Hello world" 这个字符串属于 ASCII 子集, 而 ÀÈÌÒÙ 不属于ASCII码[2]，但属于binary。
// 所以btoa和atob其实还涉及了编码问题，我们只需要找出相同的编码进行替换即可。在nodejs环境中，提供了一个buffer类，用于操作二进制及base64转码，而在Python中有一个Latini编码与Js的binary相同

var s1 = new Buffer.from("待解码的字符","base64").toString("binary")
var s2 = new Buffer.from("待编码的字符","binary").toString("base64")

// 对应的Python
// import base64
// atob
// s = base64.b64decode("待解码的字符".encode("utf8")).decode("latina1")
// btoa
// s = base64.b64encode("待编码的字符".encode("latina1")).decode("utf8")
