...
function st(t) {
    ...
    for (e + 1 == t.length ? (r = parseInt(t.substring(e, e + 1), 16),
            ...
            s += bt.charAt(r >> 2) + bt.charAt((3 & r) << 4));
        (3 & s.length) > 0;)
        s += St;
    return s
}
...
var it, nt = 0xdeadbeefcafe,
    ot = 15715070 == (16777215 & nt);
...
var at, ut, pt = "0123456789abcdefghijklmnopqrstuvwxyz",
    ct = new Array;
...
encryptPassword = function (e) {
    if (!JSEncrypt)
        return console.log("请先使用loadEncryptJs加载JS加密库");
